    ...
    return {
        ...
        drawY: function (ctx, w, h, yOpt, xais) {
            ...
            function drawYUnit() {
                ...
                for(var i = 1; i < row; i++){
                    var y = startY + step * i;
                    if(showLabel){
                      var label = (max - labelStep * i).toFixed(Y_AXIS_NUMBER_PRECISION);
                      // bitson
                      label = this.autoRoundNumPrecision(label)
                        if(i < middleIndex){
        ...
        autoRoundNumPrecision: function (value) {
          ...
          if (arr[0] && Number(arr[0]) != 0) {
            ...
            return Number(value).toFixed(Y_AXIS_NUMBER_PRECISION)
          }
        },
